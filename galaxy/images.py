...
class CUB( Image ):
    def sniff(self, filename):
        """Determine if the file is in cub format.
        """
        headers = get_headers(filename, None, 1)
        try:
            if headers[0][0].startswith("Object = IsisCube"):
                return True
            else:
                return False
        except IndexError:
...
class Cub( Image ): ## returns true for testing purposes
    def sniff(self, filename, image=None):
        return check_image_type( filename, ['cub'], image )
	return True

class IMG( Image ): ## returns true for testing purposes
    def sniff(self, filename, image=None):
        return check_image_type( filename, ['IMG'], image )
	return True

class IMQ( Image ): ## returns true for testing purposes
    def sniff(self, filename, image=None):
        return check_image_type( filename, ['IMQ'], image )
	return True
